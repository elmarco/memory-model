...
impl<'a> Bytes<usize> for VolatileSlice<'a> {
    ...
    /// #   let mut mem_ref = &mut mem[..];
    /// #   let vslice = mem_ref.as_volatile_slice();
    ///     let res = vslice.write_obj(55u64, 16);
    ///     assert!(res.is_ok());
    /// ```
    fn write_obj<T: DataInit>(&self, val: T, addr: usize) -> Result<()> {
        unsafe {
            // Guest memory can't strictly be modeled as a slice because it is
            // volatile.  Writing to it with what compiles down to a memcpy
            // won't hurt anything as long as we get the bounds checks right.
            self.region_end(addr, size_of::<T>())?;
            std::ptr::write_volatile(
                &mut self.as_mut_slice()[addr..] as *mut _ as *mut T,
                val,
            );
            Ok(())
        }
    ...
    /// #   let mut mem_ref = &mut mem[..];
    /// #   let vslice = mem_ref.as_volatile_slice();
    ///     let res = vslice.write_obj(55u64, 32);
    ///     assert!(res.is_ok());
    ///     let num: u64 = vslice.read_obj(32).unwrap();
    ///     assert_eq!(55, num);
    /// ```
    fn read_obj<T: DataInit>(&self, addr: usize) -> Result<T> {
        self.region_end(addr, size_of::<T>())?;
        unsafe {
            ...
            Ok(std::ptr::read_volatile(
                &self.as_slice()[addr..] as *const _ as *const T,
            ))
        }
    }
